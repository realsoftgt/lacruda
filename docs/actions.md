...
All `LacrudaAction`s are declared inside of your model `actions()` method e.g.:

    public function actions()
    {
        return [
            LacrudaAction::make('vehicles.actions.mark_sold'),
...
LACRUDA comes with a few default actions which can be specified:
