...
$(document).ready(function () {
    ...
    if (form.data("ajax-form") !== "submitted") {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.hasOwnProperty("show_alert"))
            show_alert(response.show_alert[0], response.show_alert[1]);
          if (response.hasOwnProperty("show_modal"))
            show_modal(response.show_modal);
          if (response.hasOwnProperty("dismiss_modal"))
            form.closest("[data-ajax-modal]").modal("toggle");
          if (response.hasOwnProperty("jquery"))
            $(response.jquery.selector)[response.jquery.method](
              response.jquery.content
            );
        },
  ...
  $(document).on("change", "[data-checkbox-id]", function () {
    ...
    let ids = $("[data-checkbox-id]:checked")
      .map(function () {
        return this.value;
      })
      .get()
