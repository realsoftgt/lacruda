...
trait LacrudaController
{
    ...
    public function index(Request $request, Builder $builder)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($actions) {
                $datatables->editColumn('action', function ($model) {
                    return view('lacruda::models.datatables.action', ['model' => $model]);
                });
            }
            if ($bulk_actions) {
                $datatables->editColumn('checkbox', function ($model) {
                    return view('lacruda::models.datatables.checkbox', ['model' => $model]);
                });
            }
        ...
        if ($bulk_actions) $html->addCheckbox(['title' => view('lacruda::models.datatables.checkbox-all')->render()]);

        return view('lacruda::models.index', ['model' => $this->model, 'html' => $html]);
    }

    public function create()
    {
        return view('lacruda::models.create', ['model' => $this->model]);
    }
    ...
    {
        return view('lacruda::models.show', ['model' => $this->model->findOrFail($id)]);
    }

    public function edit($id)
    {
        return view('lacruda::models.edit', ['model' => $this->model->findOrFail($id)]);
    }
